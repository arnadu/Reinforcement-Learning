...
#Unlike the original Agent which runs a single unit, Agent_2_1 will run one unit per factory; each unit is tied to the factory that created it
#however it uses the same NN (obs space is 14-long) as trained in a single unit mode
...
#should be the same NN as in ppo_train_1.py, with a 14-long unit observation vector
