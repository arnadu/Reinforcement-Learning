...
def obs_for_units(obs, player, units_factories, env_cfg):
    ...
    for k in units.keys():
        ...
        unit_vec = np.concatenate([pos, cargo_vec, [unit_type]], axis=-1)
...
class CustomEnvWrapper(gym.Wrapper):
    ...
    def step(self, action):  #{agent: action_0, opp_agent: action_1} where action_0 and action_ are arrays (one action for every unit of each player)
        ...
        if not done:
            obs_1, truncated = obs_for_units(obs, 'player_0', self.units_factories['player_0'], self.env.env_cfg)
            if truncated: done=True
            
            obs_2 = obs_for_units(obs, 'player_1', self.units_factories['player_1'], self.env.env_cfg)
            if truncated: done=True
        ...
        metrics["ore_produced"] = stats["generation"]["ore"]
        metrics["units"] = stats["generation"]["built"]["Light"]
        ...
        if self.prev_step_metrics is not None:
            ...
            ore_produced_this_step = (metrics["ore_produced"] - self.prev_step_metrics["ore_produced"])

            # we reward water production more as it is the most important resource for survival
            reward = ice_dug_this_step / 100 + water_produced_this_step + 10 * ore_produced_this_step + 100 * lichen_produced_this_step
...
#            next_obs, next_done = torch.Tensor(next_obs).to(device), torch.Tensor(done).to(device)
            ...
            for item in next_info: #one per env in envs
                if "episode" in item.keys(): #"episode" metrics are automatically returned by the env wrapper RecordEpisodeStatistics when an episode ends
                    ...
                    writer.add_scalar('charts/episodic_ore_produced',                   item["metrics"]["ore_produced"],                global_step)
                    ...
                    writer.add_scalar('charts/episodic_units',                          item["metrics"]["units"],                       global_step)
