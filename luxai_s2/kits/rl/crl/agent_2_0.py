...
#Unlike the original Agent which runs a single unit, Agent_2 will run one unit per factory; each unit is tied to the factory that created it
#however it uses the same NN (obs space is 13-long) as trained in a single unit mode
#this version of the Agent is not using the SimpleUnitDiscreteObservationWrapper at all; instead is is calling a new function obs_for_units() defined below; 
#it also relies on a multi_agent version of action masks from SimpleUnitDiscreteController, triggered by an extra param in 

#should be the same NN as in ppo_train.py, with a 13-long unit observation vector
MODEL_WEIGHTS_RELATIVE_PATH = "models/LuxAI_S2-v0__ppo_train__1__1678596259"
...
class Agent:
    ...
    def obs_for_units(self, obs):
        ...
        for k in units.keys():
            ...
            obs_vec = np.zeros(13) #size of observation space for NN model
