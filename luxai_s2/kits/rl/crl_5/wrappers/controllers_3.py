...
class SimpleUnitDiscreteController_3(Controller):
    ...
    def action_to_lux_action(
        ...
        if len(units)>0:  #there are no units on the very first step, yet we have an action
            for unit_id, unit_action in enumerate(zip(units.keys(), action)):  #there should be one action per unit
                unit = units[unit_id]
                choice = unit_action
                action_queue = []
                no_op = False
    ...
    def factory_actions(self, agent: str, obs: Dict[str, Any]):
        shared_obs = obs["player_0"]
        lux_action = dict()
        units = shared_obs["units"][agent]
        ...
        for _, (factory_id, factory) in enumerate(factories.items()):
            if len(units) == 0:
                lux_action[factory_id] = 1  #each factory builds a single heavy in the very first step
            else:
                if factory['cargo']['metal']>self.env_cfg.ROBOTS['LIGHT'].METAL_COST and factory['power']>self.env_cfg.ROBOTS['LIGHT'].POWER_COST:
                    lux_action[factory_id] = 0 #then they build light units as long as there are resources to do so

        #NOTE: the following will override the creation of units; but it is not a problem because it happens only at the very end of the episode
        ...
        for factory_id in factories.keys():
            ...
            if 1000 - obs[agent]['real_env_steps'] < 50 and factory["cargo"]["water"] > 100:
                lux_action[factory_id] = 2 # water and grow lichen at the very end of the game

       
        return lux_action

    def actions_to_lux_actions(self, agent: str, obs: Dict[str, Any], actions):
        #to handle multiple units
        #actions is a list of actions computed by the NN, in the same sort order as the observation's units dictionary
        #the function returns a dictionary of actions formatted to pass back to luxai_s2
        shared_obs = obs["player_0"]
        lux_action = dict()
        units = shared_obs["units"][agent]
        units_keys = list(units)
        
        for _, (unit_id, action) in enumerate(actions.items()):
            unit = units[unit_id]
            choice = action
            action_queue = []
            no_op = False
    ...
    def action_masks(self, agent: str, obs: Dict[str, Any], multi_agent=False): #added multi_agent for agent_1.py, to handle multiple units
        """
