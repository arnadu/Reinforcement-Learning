...
class AgentNN(nn.Module):
    def __init__(self, envs):
        ...
        self.actor = nn.Sequential(
            ...
            nn.Tanh(),
            layer_init(nn.Linear(ACTOR_NEURONS, envs.single_action_space.n), std=0.01),
        )
    ...
    def get_action(self, x):  #used for eval only
        c_i = self.get_combined( x )  
...
class Trainee():
    ...
    def run_step(self, flat_obs, units_ids):
        ...
        keys = units_ids[flat_obs[:,2].cpu()==self.model_filter] #matching keys of the selected units  #CPU
        ...
        with torch.no_grad():
            ...
            for i in range(self.args.num_envs): #iterate on the parallel envs
                ...
                selected = [0] #np.random.choice(np.count_nonzero(player_0), size=1) #select one unit from player_0 of this env
                ...
                action_0={}
                e_c = e.cpu()
                player_0_c = player_0.cpu()
                action_c = action.cpu()
                for _,(k,a) in enumerate(zip(keys[e_c][player_0_c], action_c[e_c][player_0_c])):  #CPU
                    if k != '':
                        action_0[k]=a

                action_1={}
                player_1_c = player_1.cpu()
                for _,(k,a) in enumerate(zip(keys[e_c][player_1_c], action_c[e_c][player_1_c])):  #CPU
                    if k != '':
                        action_1[k]=a
    ...
    def pick_one_obs(self, next_obs):
        ...
        for i, o in enumerate(next_obs): #one set of observations per parallel env
            ...
            selected = [0] #np.random.choice(units.shape[0], size=1) #select one unit from player_0 of this env
...
if __name__ == "__main__":
    ...
    args = parse_args()
    run_name = f"{args.env_id}__{args.exp_name}__{args.seed}__{time.strftime('%d-%b-%Y-%H.%M.%S', time.localtime())}"
