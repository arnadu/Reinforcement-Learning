...
class SimpleUnitDiscreteController(Controller):
    ...
    def action_to_lux_action(
        self, agent: str, obs: Dict[str, Any], action: npt.NDArray, multi_unit=False  #added multi_unit to handle multiple units in agent-1.py
    ):       
        ...
        for unit_id in units.keys():
            ...
            if not multi_unit:
                break  #remove break for agent-1
    ...
    def actions_to_lux_actions(
        self, agent: str, obs: Dict[str, Any], actions: Dict[str, int] 
    ):
        #to be used by agent-1.py to handle multiple units
        #actions is a dictionary of actions computed by the NN, keyed on the unit's ids
        #the function returns a dictionary of actions formatted to pass back to luxai_s2
        shared_obs = obs["player_0"]
        lux_action = dict()
        units = shared_obs["units"][agent]
        
        for i, (unit_id,action) in enumerate(actions.items()):
            unit = units[unit_id]
            choice = action
            action_queue = []
            no_op = False
    ...
    def action_masks(self, agent: str, obs: Dict[str, Any], multi_agent=False): #added multi_agent for agent_1.py, to handle multiple units in Agent_2
        """
